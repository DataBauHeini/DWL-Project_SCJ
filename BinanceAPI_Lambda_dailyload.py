...
def lambda_handler(event, context):
    ...
    def retrieve_data(trading_pair, step_size, save=False):
        ...
        if os.path.isfile('/tmp/' + docname):
            logger.info("File is available")
            dataframe = pd.read_csv('/tmp/' + docname)
        else:
        ...
        full_info = pd.DataFrame(candlesticks,
    ...
    for filename in files:
        df = pd.read_csv('/tmp/' + filename, index_col=None)
